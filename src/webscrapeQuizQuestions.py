...
def get_all_links(main_url, quiz_url, link_list):
    ...
    pagination_links = [a.get('href') for a in find_next_pagination_link.find_all('a', href=True)]
    ...
    section_links = [a.get('href') for a in find_next_section_link.find_all('a', href=True)]
...
def extract_question_info(soup):
    for div in soup.find_all('div', class_='bix-div-container'):
        ...
        for td in options_answer_item.find_all('tr'):
            option_item = td.find('td', {'class': 'bix-td-option', 'width': '99%'})
